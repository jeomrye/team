...
CREATE TABLE member (
    ...
    companynumber NUMBER default null, 
...
--회원 외래키
ALTER TABLE member ADD CONSTRAINT FK_member_company
		FOREIGN KEY (
			companynumber
		)
		REFERENCES company (
...
--쿠폰 외래키
alter table coupon add constraint fk_member_coupon foreign key (userid) references member(userid);
...
--쿠폰 외래키
alter table attach add constraint fk_coupon_attach foreign key (couponNumber) references coupon(couponNumber);
...
--Q&A 답변 외래키
alter table QuestionAndAnswer_reply add constraint fk_reply_QuestionAndAnswer foreign key(questionNo) references QuestionAndAnswer(questionNo);
